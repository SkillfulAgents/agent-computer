...
${Object.entries(SUBCOMMANDS).map(([cmd, subs]) =>
  ...
  if [[ "$cur" == -* ]]; then
    local cmd_flags=""
    case "$cmd" in
${Object.entries(COMMAND_FLAGS).filter(([, flags]) => flags.length > 0).map(([cmd, flags]) =>
    `      ${cmd}) cmd_flags="${flags.join(' ')}";;`
  ).join('\n')}
    esac
...
${Object.entries(SUBCOMMANDS).map(([cmd, subs]) =>
  ...
  case "$cmd" in
${Object.entries(COMMAND_FLAGS).filter(([, flags]) => flags.length > 0).map(([cmd, flags]) =>
    `    ${cmd}) _arguments ${flags.map(f => `'${f}[${flagDescription(f)}]'`).join(' ')} $global_flags ;;`
  ).join('\n')}
...
export function installCompletions(homeOverride?: string): string {
  if (platform() === 'win32') {
    return 'Shell completions are not supported on Windows (PowerShell completion coming soon)';
  }

  const home = homeOverride || homedir();
  const completionsDir = join(home, '.agent-computer', 'completions');
  mkdirSync(completionsDir, { recursive: true });

  // Write zsh completion
  const zshFile = join(completionsDir, '_agent-computer');
  writeFileSync(zshFile, generateZshCompletion());

  // Write bash completion
  const bashFile = join(completionsDir, 'agent-computer.bash');
  writeFileSync(bashFile, generateBashCompletion());
  ...
  if (!rcContainsTag(zshrc)) {
    appendFileSync(zshrc, zshSnippet);
...
function rcContainsTag(rcPath: string): boolean {
...
function bashRcName(): string {
  const home = homedir();
  if (existsSync(join(home, '.bashrc'))) return '.bashrc';
  return '.bash_profile';
}
